class ArchiveApp {
  ...
  createGalleryItem(item) {
      ...
      let primaryDisplayTag = "";
      let secondaryDisplayTag = "";
      ...
      if (itemMetaFilter) {
          primaryDisplayTag = itemMetaFilter;
      }

      if (item.species && item.species.length > 0) {
          secondaryDisplayTag = item.species[0];
      }

      let finalDisplayTag = "";
      if (primaryDisplayTag && secondaryDisplayTag) {
          finalDisplayTag = `${primaryDisplayTag}, ${secondaryDisplayTag}`;
      } else if (primaryDisplayTag) {
          finalDisplayTag = primaryDisplayTag;
      } else if (secondaryDisplayTag) {
          finalDisplayTag = secondaryDisplayTag;
      }
      ...
      infoContainer.innerHTML = `
          ...
          ${finalDisplayTag ? `<div class="gallery-item-filter ${filterClass}">${finalDisplayTag}</div>` : ""}
      `;
